    ...
    def savefile(self, iter):
        for i in range(self.rowSize):
            if (self.txt_x0[i,4] == 0):
                ...
                break
        np.savetxt(f"Imgs/{self.map_name}/{self.TESTMODE}/{str(iter)}.csv", self.txt_x0, delimiter = ',', header="laptime, ego_x_pos, ego_y_pos, actual speed, expected speed, tracking error, noise", fmt="%-10f")
